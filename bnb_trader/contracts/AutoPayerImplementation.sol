...
/**
 * @title AutoPayerImplementation - Trustless P2P Bank Transfer Facilitator
 * @dev Implementation contract for use with transparent proxy pattern
 */
contract AutoPayerImplementation is ReentrancyGuard, Ownable, Pausable {
    
    // Storage layout - NEVER CHANGE THE ORDER OF THESE VARIABLES
    // This is critical for proxy upgradability
    ...
    // State variables - NEVER CHANGE THE ORDER
    ...
    event Upgraded(uint256 indexed oldVersion, uint256 indexed newVersion);
    ...
    modifier onlyManager() {
        require(msg.sender == manager, "Not authorized manager");
        _;
    }
    ...
    constructor() Ownable(msg.sender) {
        // Implementation constructor - this will not be called when used via proxy
        // We set some default values to make the implementation contract safe
        version = 1;
        nextRequestId = 1;
    }
    
    /**
     * @dev Initialize the contract (called by proxy on deployment)
    ...
    function initialize(
        ...
        require(nextRequestId == 0, "Already initialized");
    ...
    function createEscrowRequest(
        ...
        require(oracle.supportedTokens(_tokenAddress), "Token not supported");
        require(oracle.supportedCurrencies(_fiatCurrency), "Currency not supported");
        require(oracle.isValidEscrowAmount(_tokenAmount), "Invalid escrow amount");
        require(_fiatAmount > 0, "Fiat amount must be greater than 0");
        require(bytes(_bankDetails).length > 0, "Bank details required");
        
        // Check exchange rate is available
        (uint256 rate, bool isValid) = oracle.getExchangeRate(_fiatCurrency, _tokenAddress);
        require(isValid, "Exchange rate not available or too old");
        
        // Calculate expected token amount based on current rate
        uint256 expectedTokenAmount = oracle.calculateTokenAmount(_fiatCurrency, _tokenAddress, _fiatAmount);
        require(_tokenAmount >= expectedTokenAmount * 95 / 100, "Token amount too low"); // 5% tolerance
        require(_tokenAmount <= expectedTokenAmount * 105 / 100, "Token amount too high"); // 5% tolerance
        ...
        escrowRequests[requestId] = EscrowRequest({
            ...
            expiresAt: block.timestamp + oracle.escrowDuration(),
            paidAt: 0,
            disputed: false
        });
    ...
    /**
     * @dev Submit proof of bank transfer
     */
    ...
    /**
     * @dev Verify receipt and release funds
     */
        ...
        nonReentrant 
    {
        ...
        if (_approved) {
            // Calculate platform fee using oracle
            uint256 platformFee = (request.tokenAmount * oracle.platformFeeRate()) / 10000;
    ...
    /**
     * @dev Cancel an open escrow request
     */
    ...
    // Manager functions
    function managerPause() external onlyManager {
        _pause();
    }
    
    function managerUnpause() external onlyManager {
        _unpause();
    }
    
    function managerAddVerifier(address _verifier) external onlyManager {
        verifiers[_verifier] = true;
    }
    
    function managerRemoveVerifier(address _verifier) external onlyManager {
        verifiers[_verifier] = false;
    }
    
    // Upgrade function (only for version tracking)
    function upgradeVersion() external onlyOwner {
        uint256 oldVersion = version;
        version = version + 1;
        emit Upgraded(oldVersion, version);
    }
