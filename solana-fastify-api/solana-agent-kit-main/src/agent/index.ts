...
export class SolanaAgentKit {
  ...
  constructor(
    walletAddressOrPrivateKey: string, 
    rpc_url: string, 
    config: Config, 
    isUiMode: boolean
  );
  constructor(
    ...
    this.isUiMode = isUiMode || false;

    if (!this.isUiMode) {
      if(private_key && !this.isValidPublicKey(private_key)) {
        this.wallet = Keypair.fromSecretKey(bs58.decode(private_key));
        this.wallet_address = this.wallet.publicKey;
      } else if (private_key && this.isValidPublicKey(private_key)) {
        this.wallet_address = new PublicKey(private_key);
      } else {
        throw new Error("Private key is required when not in UI mode.");
      }
    } else {
      if (private_key && this.isValidPublicKey(private_key)) {
        this.wallet_address = new PublicKey(private_key);
      } else if (private_key) {
        throw new Error("Invalid wallet address provided for UI mode.");
      }
    }
