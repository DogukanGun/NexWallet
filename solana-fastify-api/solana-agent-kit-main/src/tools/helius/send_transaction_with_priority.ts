...
export async function sendTransactionWithPriorityFee(
  ...
  try {
    if (!splmintAddress) {
      ...
      const data: { result?: { priorityFeeEstimate: number }; error?: { message?: string } } = await response.json();
      if (data.error) {
        throw new Error(`Error fetching priority fee: ${data.error.message || 'Unknown error'}`);
      }
      if (!data.result) {
        throw new Error("Error fetching priority fee: No result in response");
      }
