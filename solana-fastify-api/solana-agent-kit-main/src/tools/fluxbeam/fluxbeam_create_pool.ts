...
export async function fluxBeamCreatePool(
  ...
  try {
    ...
    ).json() as { error?: string | { message?: string; code?: number } } & { transaction: string };
    if (response.error) {
      const errorMessage = typeof response.error === 'string' ? response.error : (response.error?.message || 'Unknown error');
