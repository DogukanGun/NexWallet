...
export async function depositToDriftUserAccount(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to deposit to user account: ${error.message}`);
  }
}
...
export async function withdrawFromDriftUserAccount(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to withdraw from user account: ${error.message}`);
  }
}
...
export async function doesUserHaveDriftAccount(agent: SolanaAgentKit) {
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to check user account: ${error.message}`);
  }
}
...
export async function stakeToDriftInsuranceFund(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to get APYs: ${error.message}`);
  }
}
...
export async function requestUnstakeFromDriftInsuranceFund(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to unstake from insurance fund: ${error.message}`);
  }
}
...
export async function calculatePerpMarketFundingRate(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(
      `Something went wrong while trying to get the market's funding rate. Here's some more context: ${error.message}`,
    );
  }
}
...
export async function getL2OrderBook(marketSymbol: `${string}-PERP`) {
  try {
    const serializedOrderbook = await (
      await fetch(
...
export async function getEntryQuoteOfPerpTrade(
  ...
  try {
    ...
    const error = e as { message: string };
    throw new Error(`Failed to get entry quote: ${error.message}`);
  }
}
