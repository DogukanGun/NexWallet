...
export async function checkDebridgeTransactionStatus(
  ...
  const responseData = await orderIdsResponse.json() as deBridgeOrderIdsResponse & {
    errorCode?: number;
  ...
  if (!orderIdsResponse.ok) {
    if (responseData.errorCode === 2) {
      ...
      const errError = responseData.errorMessage as (string | { message?: string } | undefined);
      const message = typeof errError === 'string' ? errError : (errError?.message || 'Unknown error');
  ...
  const statuses = await Promise.all(
    orderIdsData.orderIds.map(async (orderId) => {
      ...
      const statusData = await statusResponse.json() as deBridgeOrderStatusResponse & { error?: string | { message?: string; code?: number } };
