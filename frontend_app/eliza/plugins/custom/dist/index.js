...
var WalletProvider = class _WalletProvider {
  ...
  getTestnetTokenAddress(tokenSymbol) {
    ...
    if (TESTNET_TOKEN_ADDRESSES2[normalizedSymbol]) {
      elizaLogger.debug(
        `Found testnet token address for ${normalizedSymbol}: ${TESTNET_TOKEN_ADDRESSES2[normalizedSymbol]}`
      );
...
var SwapAction = class {
  ...
  async confirmSwapTransaction(params) {
    const chainConfig = this.walletProvider.getChainConfigs(params.route.fromChainId);
    return await this.walletProvider.confirmTransaction({
      type: "contract",
      from: this.walletProvider.getAddress(),
      to: params.route.steps[0].tool,
      // First step's contract
      value: params.amount,
      data: `Swap ${params.amount} ${params.fromToken} for ${params.toToken} (estimated)`,
      chainId: chainConfig.id
    });
  }
  async swap(params) {
    ...
    if (!params.fromToken.startsWith("0x")) {
      try {
        ...
        elizaLogger2.error(
          `Error resolving from token address for ${params.fromToken}:`,
          error
        );
        throw new Error(
    ...
    try {
      ...
      const confirmed = await this.confirmSwapTransaction({
        fromToken: params.fromToken,
        toToken: params.toToken,
        amount: params.amount,
      ...
      if (!confirmed) {
        throw new Error("Swap transaction rejected by user");
      }
      elizaLogger2.debug(
        `Executing route: ${JSON.stringify(routes.routes[0].steps, null, 2)}`
      );
  ...
  validateAndNormalizeParams(params) {
    ...
    try {
      ...
      throw new Error(
        `Invalid swap amount: ${params.amount}. Please provide a valid number.`
      );
    }
    if (params.slippage !== void 0) {
      ...
      if (params.slippage <= 0 || params.slippage > 1) {
        elizaLogger2.error(
          `Invalid slippage value: ${params.slippage} (must be between 0 and 1)`
        );
...
var swapAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const _promptLower = promptText.toLowerCase();
    ...
    const chain = content.chain?.toLowerCase() || "bsc";
...
import { formatEther, formatUnits as formatUnits2, parseEther as parseEther2, parseUnits, erc20Abi as erc20Abi2 } from "viem";
var TransferAction = class {
  // 5 Gwei
  constructor(walletProvider) {
    this.walletProvider = walletProvider;
  }
  TRANSFER_GAS = BigInt(21e3);
  DEFAULT_GAS_PRICE = BigInt(5e9);
  async confirmTransferTransaction(params) {
  ...
  async transfer(params) {
    ...
    if (!this.walletProvider.chains[params.chain]) {
      elizaLogger3.error(
        `Chain '${params.chain}' is not supported. Available chains: ${Object.keys(this.walletProvider.chains).join(", ")}`
      );
      throw new Error(
        `Chain '${params.chain}' is not supported. Please use one of: ${Object.keys(this.walletProvider.chains).join(", ")}`
      );
    }
    ...
    if (!params.token || params.token === "null" || params.token === nativeToken) {
      ...
      if (!params.amount) {
        ...
        value = balance - this.DEFAULT_GAS_PRICE * this.TRANSFER_GAS;
        options.gas = this.TRANSFER_GAS;
        options.gasPrice = this.DEFAULT_GAS_PRICE;
        transferAmount = formatEther(value);
      } else {
        value = parseEther2(params.amount);
        transferAmount = params.amount;
      }
      const confirmed = await this.confirmTransferTransaction({
        chain: params.chain,
      ...
      if (!confirmed) {
        throw new Error("Transfer transaction rejected by user");
      }
      ...
      if (params.token === "BNB" || params.token === "bnb") {
        elizaLogger3.debug(
          "Detected native token (BNB) passed to ERC20 handling branch - switching to native token handling"
        );
        ...
        if (!params.amount) {
          ...
          value2 = balance - this.DEFAULT_GAS_PRICE * this.TRANSFER_GAS;
          options.gas = this.TRANSFER_GAS;
          options.gasPrice = this.DEFAULT_GAS_PRICE;
          transferAmount2 = formatEther(value2);
        } else {
          value2 = parseEther2(params.amount);
          transferAmount2 = params.amount;
        }
        const confirmed2 = await this.confirmTransferTransaction({
          chain: params.chain,
      ...
      if (!params.amount) {
        ...
        transferAmount = formatUnits2(value, decimals);
      } else {
  ...
  async validateAndNormalizeParams(params) {
    ...
    params.data = `${params.data}` === "null" ? "0x" : params.data;
...
var ListaDaoAbi = [
  ...
  {
    inputs: [{ internalType: "uint256", name: "_annualRate", type: "uint256" }],
    name: "setAnnualRate",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function"
...
var BridgeAction = class {
  ...
  L2_BRIDGE_ADDRESS = "0x2222222222222222222222222222222222222222";
  LEGACY_ERC20_ETH = "0x4200000000000000000000000000000000000006";
  async confirmBridgeTransaction(params) {
    ...
    return await this.walletProvider.confirmTransaction({
      ...
      data: `Bridge from ${params.fromChain} to ${params.toChain}${params.toAddress ? ` (recipient: ${params.toAddress})` : ""}`,
      chainId: chainConfig.id
    });
  }
  async bridge(params) {
    elizaLogger4.debug(
      "Starting bridge operation with params:",
    ...
    if (nativeTokenBridge) {
      ...
      elizaLogger4.debug(
        `Native token amount: ${amount} wei (${params.amount} ${nativeToken})`
      );
    } else {
      ...
      elizaLogger4.debug(
        `Token amount: ${amount} (${params.amount} tokens with ${decimals} decimals)`
      );
    }
    try {
      if (params.fromChain === "bsc" && params.toChain === "opBNB") {
        ...
        const confirmed = await this.confirmBridgeTransaction({
          ...params,
          bridgeAddress: this.L1_BRIDGE_ADDRESS,
          value: amount
        });
        if (!confirmed) {
          throw new Error("Bridge transaction rejected by user");
        }
        ...
        if (selfBridge && nativeTokenBridge) {
          ...
          elizaLogger4.debug(
            `From token: ${params.fromToken}, To token: ${params.toToken}`
          );
          const args = [params.fromToken, params.toToken, amount, 1, "0x"];
          ...
          elizaLogger4.debug(
            "Bridge to another address with native token - using depositETHTo"
          );
          ...
          elizaLogger4.debug(
            "Bridge to another address with ERC20 token - using depositERC20To"
          );
          elizaLogger4.debug(
            `From token: ${params.fromToken}, To token: ${params.toToken}`
          );
          ...
          resp.txHash = await l1BridgeContract.write.depositERC20To(args, {
            account,
            chain
          });
          elizaLogger4.debug(`Transaction submitted with hash: ${resp.txHash}`);
        ...
        const confirmed = await this.confirmBridgeTransaction({
          ...params,
          bridgeAddress: this.L2_BRIDGE_ADDRESS,
          value: amount
        });
        if (!confirmed) {
          throw new Error("Bridge transaction rejected by user");
        ...
        if (selfBridge && nativeTokenBridge) {
          elizaLogger4.debug(
            "Self bridge with native token - using withdraw with LEGACY_ERC20_ETH"
          );
          const args = [this.LEGACY_ERC20_ETH, amount, 1, "0x"];
          const value = amount + delegationFee;
          elizaLogger4.debug(
            `Simulating withdraw with value: ${value} (amount + delegationFee)`
          );
          ...
          elizaLogger4.debug(
            "Bridge to another address with native token - using withdrawTo with LEGACY_ERC20_ETH"
          );
          elizaLogger4.debug(`Recipient address: ${params.toAddress}`);
          const args = [
          ...
          const value = amount + delegationFee;
          elizaLogger4.debug(
            `Simulating withdrawTo with value: ${value} (amount + delegationFee)`
          );
          ...
          elizaLogger4.debug(
            "Bridge to another address with ERC20 token - using withdrawTo"
          );
          elizaLogger4.debug(`Token: ${params.fromToken}`);
          elizaLogger4.debug(`Recipient address: ${params.toAddress}`);
          const args = [params.fromToken, params.toAddress, amount, 1, "0x"];
          const value = delegationFee;
        ...
        elizaLogger4.error(
          `Unsupported bridge direction: ${params.fromChain} to ${params.toChain}`
        );
        throw new Error(
          `Unsupported bridge direction: ${params.fromChain} to ${params.toChain}. Only BSC \u2194 opBNB is supported.`
        );
      }
      if (!resp.txHash || resp.txHash === "0x") {
      ...
      if (error.message.includes("insufficient funds")) {
        throw new Error(
          `Insufficient funds to bridge ${params.amount} ${resp.fromToken}. Please check your balance.`
        );
      }
      ...
      if (error.message.includes("execution reverted")) {
        throw new Error(
          "Bridge transaction reverted. This could be due to contract restrictions or incorrect parameters."
        );
      }
      throw error;
    }
  ...
  async validateAndNormalizeParams(params) {
    ...
    try {
      ...
      if (Number.isNaN(amountValue) || amountValue <= 0) {
        ...
        throw new Error(
          `Invalid amount: ${params.amount}. Please provide a positive number.`
        );
      }
      ...
      throw new Error(
        `Invalid amount format: ${params.amount}. Please provide a valid number.`
      );
    }
    ...
    if (!params.toAddress) {
      ...
      elizaLogger4.debug(
        `No valid toAddress provided, using wallet address: ${params.toAddress}`
      );
    } else {
    ...
    if (params.fromChain === "bsc" && params.toChain === "opBNB") {
      ...
      if (params.fromToken && !params.fromToken.startsWith("0x")) {
        ...
        throw new Error(
          `Invalid token address format: ${params.fromToken}. Must start with 0x.`
        );
      }
      if (params.toToken && !params.toToken.startsWith("0x")) {
        elizaLogger4.error(`Invalid toToken address format: ${params.toToken}`);
        throw new Error(
          `Invalid token address format: ${params.toToken}. Must start with 0x.`
        );
      }
    }
...
var bridgeAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (match && match.length >= 3) {
      ...
      elizaLogger4.debug(
        `Directly extracted BSC to opBNB bridge - Amount: ${directAmount}, Token: ${directToken}`
      );
    } else {
      ...
      if (match && match.length >= 3) {
        ...
        directToken = match[2].toUpperCase();
        elizaLogger4.debug(
          `Directly extracted opBNB to BSC bridge - Amount: ${directAmount}, Token: ${directToken}`
        );
      }
    }
    ...
    if (directToken && fromChain) {
      if (directToken !== "BNB" && directToken.startsWith("0x")) {
        ...
        elizaLogger4.debug(
          `Using token address directly extracted from prompt: ${fromToken}`
        );
      } else {
        fromToken = void 0;
    ...
    if (content.toToken) {
      ...
      if (toToken) {
        ...
        elizaLogger4.debug(
          "Content contained null/invalid toToken, using undefined instead"
        );
      }
    }
    ...
    if (directToAddress?.startsWith("0x")) {
      ...
      if (toAddress) {
        ...
        elizaLogger4.debug(
          "Content contained null/invalid toAddress, using undefined instead"
        );
      }
    }
    const walletProvider = initWalletProvider(runtime);
...
var DeployAction = class {
  ...
  async deployERC20(deployTokenParams) {
    elizaLogger6.debug(
      "Deploying ERC20 token with params:",
      JSON.stringify(deployTokenParams, null, 2)
    );
    ...
    elizaLogger6.debug(
      `Deploying ERC20 token: ${name} (${symbol}) with ${decimals} decimals and total supply ${totalSupply}`
    );
    try {
      elizaLogger6.debug(
        `Converting total supply ${totalSupply} to wei with ${decimals} decimals`
      );
      ...
      const contractAddress = await this.deployContract(chain, "ERC20Contract", args);
      if (!contractAddress) {
        elizaLogger6.error("Failed to deploy ERC20 contract - no address returned");
        throw new Error("Failed to deploy ERC20 contract");
      }
      elizaLogger6.debug(
        `ERC20 contract deployed successfully at address: ${contractAddress}`
      );
      return {
  ...
  async deployERC721(deployNftParams) {
    elizaLogger6.debug(
      "Deploying ERC721 NFT with params:",
      JSON.stringify(deployNftParams, null, 2)
    );
    ...
    try {
      ...
      const contractAddress = await this.deployContract(chain, "ERC721Contract", args);
      if (!contractAddress) {
        elizaLogger6.error("Failed to deploy ERC721 contract - no address returned");
        throw new Error("Failed to deploy ERC721 contract");
      }
      elizaLogger6.debug(
        `ERC721 contract deployed successfully at address: ${contractAddress}`
      );
      return {
        address: contractAddress
      };
  ...
  async deployERC1155(deploy1155Params) {
    elizaLogger6.debug(
      "Deploying ERC1155 token with params:",
      JSON.stringify(deploy1155Params, null, 2)
    );
    ...
    try {
      ...
      const contractAddress = await this.deployContract(chain, "ERC1155Contract", args);
      if (!contractAddress) {
      ...
      elizaLogger6.debug(
        `ERC1155 contract deployed successfully at address: ${contractAddress}`
      );
      return {
        address: contractAddress
      };
  ...
  async deployContract(chain, contractName, args) {
    elizaLogger6.debug(
      `Starting contract deployment process for ${contractName} on chain ${chain}`
    );
    try {
      ...
      if (receipt.status === "success") {
        elizaLogger6.debug(
          `Contract deployed successfully at address: ${receipt.contractAddress}`
        );
      } else {
      ...
      if (error.message.includes("insufficient funds")) {
        throw new Error(
          "Insufficient funds to deploy the contract. Please check your balance."
        );
      }
      if (error.message.includes("user rejected")) {
        throw new Error("Transaction rejected by user.");
...
var deployAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (match && match.length >= 2) {
      directDecimals = Number.parseInt(match[1], 10);
    ...
    if (directContractType) {
      ...
      elizaLogger6.debug(
        `Using contract type directly extracted from prompt: ${contractType}`
      );
    ...
    try {
      elizaLogger6.debug(
        `Starting deployment process for ${contractType.toUpperCase()} contract on ${chain}...`
      );
      let result;
      switch (contractType.toLowerCase()) {
        case "erc20": {
          ...
          elizaLogger6.debug(
            `Deploying ERC20 with params: name=${name}, symbol=${symbol}, decimals=${decimals}, totalSupply=${totalSupply}`
          );
        ...
        case "erc721": {
          ...
          elizaLogger6.debug(
            `Deploying ERC721 with params: name=${nftName}, symbol=${nftSymbol}, baseURI=${nftBaseURI}`
          );
        ...
        case "erc1155": {
          ...
          elizaLogger6.debug(
            `Deploying ERC1155 with params: name=${multiName}, baseURI=${multiBaseURI}`
          );
        ...
        default:
          ...
          throw new Error(
            `Unsupported contract type: ${contractType}. Supported types are: erc20, erc721, erc1155`
          );
      }
      if (result?.address) {
...
var GetBalanceTestnetAction = class {
  constructor(walletProvider) {
    this.walletProvider = walletProvider;
    elizaLogger7.debug("GetBalanceTestnetAction initialized with provider:", {
    ...
    try {
      ...
      if (token) {
        if (token.startsWith("0x") && token.length === 42) {
          try {
            const publicClient = this.walletProvider.getPublicClient(chain);
            elizaLogger7.debug(`Public client for ${chain}:`, {
            ...
            const symbol = await publicClient.readContract({
              ...
              elizaLogger7.debug(
                `Failed to get symbol for token ${token}:`,
                e.message
              );
              return null;
            });
          ...
          elizaLogger7.debug(
            `Token symbol ${token} maps to address:`,
            mappedAddress || "Not found in mapping"
          );
          if (!mappedAddress) {
            elizaLogger7.error(
    ...
    if (!queryNativeToken) {
      ...
      if (isAddress(token)) {
        ...
        if (!normalizedAddress) {
          throw new Error(
            `Invalid token address format: ${token}. Please provide a valid token address or symbol.`
          );
        }
        ...
        if (!mappedAddress) {
          ...
          throw new Error(
            `Token ${token} is not supported on BSC testnet. Supported tokens: ${SUPPORTED_TESTNET_TOKENS.join(", ")}`
          );
        }
      ...
      elizaLogger7.debug(
        `Getting ERC20 balance for address ${address} and token ${tokenAddress}`
      );
      try {
      ...
      try {
        ...
        if (nativeBalanceWei === 0n) {
          ...
          if (bscScanBalance && Number.parseFloat(bscScanBalance) > 0) {
            elizaLogger7.debug(
              `BSCScan reports non-zero balance: ${bscScanBalance} BNB`
            );
            resp.balance = {
              token: nativeSymbol,
  ...
  async getERC20TokenBalance(chain, address, tokenAddress) {
    try {
      elizaLogger7.debug(
        `Getting ERC20 token balance for address ${address} and token ${tokenAddress} on chain ${chain}`
      );
      const publicClient = this.walletProvider.getPublicClient(chain);
      ...
      try {
        ...
        elizaLogger7.warn(
          `Token ${tokenAddress} might not exist on BSC testnet or isn't a valid ERC20 token`
        );
        return "0";
      }
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      if (config.BSC_TESTNET_PROVIDER_URL === "https://data-seed-prebsc-2-s3.bnbchain.org:8545") {
        elizaLogger7.debug(
          "Using default RPC URL. Consider using a more reliable one like https://data-seed-prebsc-2-s3.bnbchain.org:8545/"
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (tokenInput) {
        ...
        if (tokenInput.startsWith("0x")) {
          ...
          if (normalizedAddress) {
            ...
            if (mappedAddress) {
              elizaLogger7.debug(
                `Found token symbol in mapping despite 0x prefix: ${upperToken}`
              );
          ...
          if (mappedAddress) {
            elizaLogger7.debug(
              `Mapped token symbol ${tokenInput} to address: ${mappedAddress}`
            );
            tokenInput = mappedAddress;
          } else {
      ...
      try {
        elizaLogger7.debug(
          `Attempting to get balance for token: ${getBalanceOptions.token}`
        );
        if (typeof getBalanceOptions.token === "string" && getBalanceOptions.token.startsWith("0x") && getBalanceOptions.token.length === 42) {
          ...
          elizaLogger7.debug(
            `Using previously mapped token address: ${getBalanceOptions.token}`
          );
        }
...
${displayToken}: ${getBalanceResp.balance.amount}`;
        ...
        } else if (originalToken?.startsWith("0x")) {
      ...
      elizaLogger7.error(
        "Critical error in getBalanceTestnet handler:",
        error.message,
        error.stack
      );
      callback?.({
  ...
  async getBalance(params) {
    ...
    if (!queryNativeToken) {
      ...
      if (token.startsWith("0x")) {
        amount = await this.getERC20TokenBalance(chain, address, token);
      } else {
        if (chainId !== 56) {
          throw new Error(
...
var StakeAction = class {
  ...
  validateStakeParams(params) {
    elizaLogger9.debug(
      `Validating stake params: chain=${params.chain}, action=${params.action}, amount=${params.amount}`
    );
    if (!params.chain) {
    ...
    if (!params.action) {
      ...
      throw new Error(
        "Action is required for staking. Use 'deposit', 'withdraw', or 'claim'"
      );
    }
    ...
    if (!validActions.includes(params.action)) {
      ...
      throw new Error(
        `Invalid staking action: ${params.action}. Valid actions are: ${validActions.join(", ")}`
      );
    }
  ...
  async confirmStakeTransaction(params) {
    const chainConfig = this.walletProvider.getChainConfigs("bsc");
    return await this.walletProvider.confirmTransaction({
      type: "contract",
      from: this.walletProvider.getAddress(),
      to: params.contract,
      value: params.amount,
      data: `${params.action} ${params.amount || ""} ${params.action === "deposit" ? "BNB" : "slisBNB"}`,
      chainId: chainConfig.id
    });
  }
  async doDeposit(amount) {
    elizaLogger9.debug(`Starting deposit of ${amount} BNB to Lista DAO`);
    const confirmed = await this.confirmStakeTransaction({
      action: "deposit",
      amount,
      contract: this.LISTA_DAO
    });
    if (!confirmed) {
      throw new Error("Deposit transaction rejected by user");
    }
    ...
    elizaLogger9.debug(
      `Preparing to deposit ${amount} BNB with parseEther value: ${parseEther4(amount)}`
    );
    try {
...
Transaction Hash: ${txHash}`;
      ...
      if (error.message.includes("insufficient funds")) {
        throw new Error(
          `Insufficient funds to deposit ${amount} BNB. Please check your balance.`
        );
      }
      if (error.message.includes("user rejected")) {
        throw new Error("Transaction rejected by user.");
  ...
  async doWithdraw(amount) {
    ...
    if (!amount) {
      ...
      withdrawAmount = formatEther4(amountToWithdraw);
      elizaLogger9.debug(`Total slisBNB balance to withdraw: ${withdrawAmount}`);
    } else {
      withdrawAmount = amount;
      amountToWithdraw = parseEther4(amount);
      elizaLogger9.debug(
        `Withdrawing specific amount: ${amount} slisBNB (${amountToWithdraw} wei)`
      );
    }
    ...
    if (!confirmed) {
      throw new Error("Withdraw transaction rejected by user");
    }
    try {
      ...
      if (allowance < amountToWithdraw) {
        ...
        elizaLogger9.debug(
          `Increasing slisBNB allowance by ${formatEther4(neededAllowance)}`
        );
...
Transaction Hash: ${txHash}`;
      ...
      if (error.message.includes("insufficient funds") || error.message.includes("insufficient balance")) {
        throw new Error(
          "Insufficient slisBNB balance to withdraw. Please check your balance."
        );
      }
      if (error.message.includes("user rejected")) {
        throw new Error("Transaction rejected by user.");
  ...
  async doClaim() {
    elizaLogger9.debug("Starting claim of unlocked BNB from Lista DAO");
    const confirmed = await this.confirmStakeTransaction({
      action: "claim",
      contract: this.LISTA_DAO
    });
    if (!confirmed) {
      throw new Error("Claim transaction rejected by user");
    }
    const publicClient = this.walletProvider.getPublicClient("bsc");
    const walletClient = this.walletProvider.getWalletClient("bsc");
    const account = walletClient.account;
    ...
    try {
      ...
      for (let idx = 0; idx < requests.length; idx++) {
        ...
        if (isClaimable) {
          elizaLogger9.debug(
            `Request #${idx} is claimable, amount: ${formatEther4(amount)} BNB`
          );
...
var stakeAction = {
  ...
  similes: ["DELEGATE", "STAKE", "DEPOSIT", "UNDELEGATE", "UNSTAKE", "WITHDRAW", "CLAIM"]
};
  ...
  async faucet(params) {
    ...
    try {
      ...
      if (!params.toAddress) {
        params.toAddress = this.walletProvider.getAddress();
        elizaLogger10.debug(
          `No address specified, using wallet address: ${params.toAddress}`
        );
      }
      const resp = {
        token: params.token,
      ...
      try {
        ...
        resp.txHash = txHash;
        elizaLogger10.debug(
          `Faucet success: ${params.token} to ${params.toAddress}, tx: ${txHash}`
        );
        return resp;
      } finally {
        ws.close();
  ...
  async validateAndNormalizeParams(params) {
    ...
    try {
      ...
      if (!this.SUPPORTED_TOKENS.includes(params.token)) {
        throw new Error(
          `Unsupported token: ${params.token}. Supported tokens are: ${this.SUPPORTED_TOKENS.join(", ")}`
        );
      }
      if (!params.toAddress) {
        params.toAddress = this.walletProvider.getAddress();
...
var InitGnfdClient = async (runtime) => {
  ...
  const client = Client.create(config.GREENFIELD_RPC_URL, config.GREENFIELD_CHAIN_ID);
  return client;
};
var CONFIG = {
  MAINNET: {
...
var { Client: Client2, Long, VisibilityType } = require4("@bnb-chain/greenfield-js-sdk");
var GreenfieldAction = class {
  ...
  async createBucket(msg) {
    ...
    const createBucketTxRes = await createBucketTx.broadcast({
      ...
      granter: "",
      privateKey: this.walletProvider.getAccount().address
    });
  ...
  async uploadObject(msg) {
    const uploadRes = await this.gnfdClient.object.delegateUploadObject(msg, {
      type: "ECDSA",
      privateKey: this.walletProvider.getAccount().address
    });
    if (uploadRes.code === 0) {
    ...
    try {
      ...
      switch (actionType) {
        case "createBucket": {
          ...
          const _hash = await action.createBucket(msg);
        ...
        case "crossChainTransfer": {
          const hash = await action.bnbTransferToGnfd(
            parseEther5(String(content.amount)),
            runtime
          );
...
${chalk.cyan("\u250C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2510")}`);
  console.log(
    chalk.cyan("\u2502") + chalk.yellow.bold("          BNB PLUGIN             ") + chalk.cyan(" \u2502")
  );
  console.log(chalk.cyan("\u251C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2524"));
  console.log(
    chalk.cyan("\u2502") + chalk.white("  Initializing BNB Services...    ") + chalk.cyan("\u2502")
  );
  console.log(
...
${chalk.cyan("\u250C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2510")}`);
  console.log(
    chalk.cyan("\u2502") + chalk.white(" Configuration Status                 ") + chalk.cyan("\u2502")
  );
  console.log(chalk.cyan("\u251C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2524"));
  console.log(
    chalk.cyan("\u2502") + chalk.white(` BSC Provider    : ${bscProvider}                    `) + chalk.cyan("\u2502")
  );
  console.log(
    chalk.cyan("\u2502") + chalk.white(` BSC Testnet     : ${bscTestnetProvider}                    `) + chalk.cyan("\u2502")
  );
  console.log(
    chalk.cyan("\u2502") + chalk.white(` OPBNB Provider  : ${opbnbProvider}                    `) + chalk.cyan("\u2502")
  );
  console.log(
    chalk.cyan("\u2502") + chalk.white(` Wallet          : ${wallet}                    `) + chalk.cyan("\u2502")
  );
  console.log(chalk.cyan("\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518"));
