...
export class BridgeAction {
    private readonly L1_BRIDGE_ADDRESS = '0x1111111111111111111111111111111111111111' as const;
    private readonly L2_BRIDGE_ADDRESS = '0x2222222222222222222222222222222222222222' as const;
    private readonly LEGACY_ERC20_ETH = '0x4200000000000000000000000000000000000006' as const;

    constructor(private walletProvider: WalletProvider) {}

    private async confirmBridgeTransaction(params: {
        ...
        try {
            ...
            if (params.fromChain === 'bsc' && params.toChain === 'opBNB') {
                ...
                if (selfBridge && nativeTokenBridge) {
                    ...
                    const args = [params.fromToken!, params.toToken!, amount, 1, '0x'] as const;
            ...
            // Handle opBNB to BSC bridging
                ...
                if (selfBridge && nativeTokenBridge) {
                    ...
                    const args = [params.fromToken!, params.toAddress!, amount, 1, '0x'] as const;
                    const value = delegationFee;
...
export const bridgeAction = {
    ...
    handler: async (
        ...
        const depositRegex =
            /(?:deposit|bridge|send)\s+([0-9.]+)\s+([a-zA-Z0-9]+)(?:\s+from)?\s+(?:bsc|binance)(?:\s+to)\s+(?:opbnb|op)/i;
        const withdrawRegex =
            /(?:withdraw|bridge|send)\s+([0-9.]+)\s+([a-zA-Z0-9]+)(?:\s+from)?\s+(?:opbnb|op)(?:\s+to)\s+(?:bsc|binance)/i;
        ...
        if (!directFromChain || !directToChain) {
            if (
                promptLower.includes('bsc to opbnb') ||
                ...
                promptLower.includes('opbnb to bsc') ||
        ...
        const promptAnalysis = {
            ...
            containsOpBNB:
                promptLower.includes('opbnb') ||
                promptLower.includes('op bnb') ||
                promptLower.includes('op-bnb'),
        ...
        try {
            ...
            if (error.message.includes('insufficient funds')) {
                errorMessage =
                    'Insufficient funds for the bridge operation. Please check your balance and try with a smaller amount.';
            } else if (error.message.includes('user rejected')) {
                errorMessage =
                    'Transaction was rejected. Please try again if you want to proceed with the bridge operation.';
            } else if (error.message.includes('token address on opBNB is required')) {
                errorMessage =
                    'When bridging ERC20 tokens from BSC to opBNB, you must specify the token address on opBNB.';
            } else if (error.message.includes('Unsupported bridge direction')) {
                errorMessage =
                    'Only bridges between BSC and opBNB are supported. Valid directions are BSC→opBNB and opBNB→BSC.';
            }
