    ...
    async getBalance(params: GetBalanceParams): Promise<GetBalanceResponse> {
        ...
        if (!queryNativeToken) {
            ...
            if (token.startsWith('0x')) {
                amount = await this.getERC20TokenBalance(chain, address, token as `0x${string}`);
            } else {
                if (chainId !== 56) {
                    throw new Error(
    ...
    handler: async (
        ...
        try {
            ...
            if (error.message.includes('getTldInfo')) {
                ...
                userMessage =
                    'Only BSC mainnet supports looking up tokens by symbol. Please try using a token address instead.';
            } else if (error.message.includes('Invalid address')) {
                userMessage =
                    'The address provided is invalid. Please provide a valid wallet address.';
            } else if (error.message.includes('Cannot read properties')) {
                userMessage =
                    'There was an issue processing your request. Please check your inputs and try again.';
            }
