...
import type { GetBalanceParams, GetBalanceResponse, SupportedChain } from '../types';
...
export const getBalanceTestnetAction = {
    ...
    handler: async (
        ...
        try {
            ...
            if (
                config.BSC_TESTNET_PROVIDER_URL ===
        ...
        try {
            ...
            try {
                ...
                if (
                    typeof getBalanceOptions.token === 'string' &&
                ...
                if (
                    error.message.includes('getTldInfo') ||
                    error.message.includes('Only BSC mainnet supports looking up tokens')
                ) {
                    ...
                    userMessage =
                        'Network connection issue. Please check your BSC_TESTNET_PROVIDER_URL configuration.';
                } else if (error.message.includes('Invalid address')) {
                    userMessage =
                        'The address provided is invalid. Please provide a valid wallet address.';
                } else if (error.message.includes('not supported on BSC testnet')) {
                    userMessage = error.message;
                } else if (error.message.includes('Contract 0x')) {
                    userMessage =
                        'Contract error. The token contract at the given address may not be valid on BSC testnet.';
