...
export class StakeAction {
    private readonly LISTA_DAO = '0x1adB950d8bB3dA4bE104211D5AB038628e477fE6' as const;
    private readonly SLIS_BNB = '0xB0b84D294e0C75A6abe60171b70edEb2EFd14A1B' as const;
    ...
    private async confirmStakeTransaction(params: {
    ...
    async doDeposit(amount: string): Promise<string> {
        ...
        elizaLogger.debug(
            `Preparing to deposit ${amount} BNB with parseEther value: ${parseEther(amount)}`
        );
    ...
    async doWithdraw(amount?: string): Promise<string> {
        ...
        if (!amount) {
            ...
            withdrawAmount = formatEther(amountToWithdraw);
            elizaLogger.debug(`Total slisBNB balance to withdraw: ${withdrawAmount}`);
        } else {
        ...
        try {
            ...
            if (allowance < amountToWithdraw) {
                ...
                elizaLogger.debug(
                    `Increasing slisBNB allowance by ${formatEther(neededAllowance)}`
                );
    ...
    async doClaim(): Promise<string> {
        elizaLogger.debug('Starting claim of unlocked BNB from Lista DAO');
        ...
        try {
            ...
            for (let idx = 0; idx < requests.length; idx++) {
                ...
                if (isClaimable) {
                    elizaLogger.debug(
                        `Request #${idx} is claimable, amount: ${formatEther(amount)} BNB`
                    );
...
export const stakeAction = {
    ...
    handler: async (
        ...
        const stakeRegex =
            /(?:stake|deposit)\s+([0-9.]+)\s+(?:bnb|slisBNB)\s+(?:on|in|to|at)?(?:\s+lista\s+dao)?(?:\s+on)?\s+(?:bsc|binance)/i;
        const withdrawRegex =
            /(?:withdraw|unstake|undelegate)\s+([0-9.]+)\s+(?:bnb|slisBNB)\s+(?:from|on)?\s+(?:lista\s+dao)?(?:\s+on)?\s+(?:bsc|binance)/i;
        const claimRegex =
            /claim\s+(?:bnb|unlocked\s+bnb|rewards?)(?:\s+from)?\s+(?:lista\s+dao)?(?:\s+on)?\s+(?:bsc|binance)/i;
        ...
        if (!directAction) {
            if (promptLower.includes('stake') || promptLower.includes('deposit')) {
                ...
                promptLower.includes('withdraw') ||
                promptLower.includes('unstake') ||
                promptLower.includes('undelegate')
            ) {
                directAction = 'withdraw';
        ...
        try {
            ...
            if (error.message.includes('insufficient funds')) {
                errorMessage =
                    'Insufficient funds for the stake operation. Please check your balance and try with a smaller amount.';
            } else if (error.message.includes('user rejected')) {
                errorMessage =
                    'Transaction was rejected. Please try again if you want to proceed with the stake operation.';
