...
export class SwapAction {
    ...
    private async confirmSwapTransaction(params: {
        fromToken: string;
        toToken: string;
        amount: string;
        route: any; // Using any for now since the route type is complex
    }): Promise<boolean> {
        ...
        return await this.walletProvider.confirmTransaction({
            ...
            to: params.route.steps[0].tool, // First step's contract
            value: params.amount,
    ...
    handler: async (
        ...
        try {
            ...
            if (error.message.includes('No routes found')) {
                ...
                errorMessage =
                    'Insufficient funds for the swap. Please check your balance and try with a smaller amount.';
            } else if (error.message.includes('high slippage')) {
                errorMessage =
                    'Swap failed due to high price impact. Try reducing the amount or using a different token pair.';
            }
