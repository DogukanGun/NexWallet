...
import { formatEther, formatUnits, parseEther, parseUnits, erc20Abi, type Hex } from 'viem';
...
export class TransferAction {
    private readonly TRANSFER_GAS = BigInt(21000);
    private readonly DEFAULT_GAS_PRICE = BigInt(5000000000); // 5 Gwei

    constructor(private walletProvider: WalletProvider) {}

    private async confirmTransferTransaction(params: {
    ...
    async transfer(params: TransferParams): Promise<TransferResponse> {
        ...
        if (!params.token || params.token === 'null' || params.token === nativeToken) {
            ...
            if (!params.amount) {
                ...
                transferAmount = formatUnits(value, decimals);
            } else {
    ...
    handler: async (
        ...
        try {
            ...
            if (error.message.includes('LI.FI SDK')) {
                ...
                if (
                    ...
                    if (tokenValue === 'null' || tokenValue === 'undefined' || !tokenValue) {
                        errorMessage +=
                            " For BNB transfers, please explicitly specify 'BNB' as the token.";
                    }
                } else if (
                    error.message.includes('400 Bad Request') &&
                    error.message.includes('chain must be')
                ) {
            ...
            if (error.message.includes('insufficient funds')) {
                errorMessage =
                    'Insufficient funds for the transaction. Please check your balance and try again with a smaller amount.';
