...
export const deployAction = {
    ...
    handler: async (
        ...
        const erc20Regex =
            /(?:deploy|create)\s+(?:an?\s+)?(?:erc20|token)(?:\s+token)?\s+(?:with|having|named)?\s+(?:name\s+['"]?([^'"]+)['"]?|['"]?([^'"]+)['"]?\s+token)/i;
        const erc721Regex =
            /(?:deploy|create)\s+(?:an?\s+)?(?:erc721|nft)(?:\s+token)?\s+(?:with|having|named)?\s+(?:name\s+['"]?([^'"]+)['"]?|['"]?([^'"]+)['"]?\s+nft)/i;
        const erc1155Regex =
            /(?:deploy|create)\s+(?:an?\s+)?(?:erc1155|multi-token)(?:\s+token)?\s+(?:with|having|named)?\s+(?:name\s+['"]?([^'"]+)['"]?|['"]?([^'"]+)['"]?\s+token)/i;
        ...
        if (match && match.length >= 2) {
            ...
            if (directTotalSupply.endsWith('k') || directTotalSupply.endsWith('K')) {
                ...
                directTotalSupply = (
                    Number.parseFloat(directTotalSupply) * 1000000000000
                ).toString();
            }
        ...
        try {
            ...
            switch (contractType.toLowerCase()) {
                ...
                case 'erc721': {
                    ...
                    const nftBaseURI =
                        directBaseURI || content.baseURI || 'https://example.com/token/';
                ...
                case 'erc1155': {
                    ...
                    const multiBaseURI =
                        directBaseURI || content.baseURI || 'https://example.com/multi-token/';
            ...
            if (error.message.includes('insufficient funds')) {
                errorMessage =
                    'Insufficient funds for contract deployment. Please check your wallet balance.';
            } else if (error.message.includes('user rejected')) {
                errorMessage =
                    'Transaction was rejected. Please try again if you want to proceed with the deployment.';
            } else if (error.message.includes('compilation failed')) {
                errorMessage =
                    'Contract compilation failed. This might be due to syntax errors in the contract code.';
            }
