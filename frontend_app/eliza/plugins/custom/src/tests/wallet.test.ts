...
describe('Wallet provider', () => {
    ...
    describe('Constructor', () => {
        ...
        it('get chain configs', () => {
            ...
            expect(walletProvider.getChainConfigs('opBNB').id).toEqual(opBNB.id);
        });
    });
    describe('Clients', () => {
        ...
        it('generates public client with custom rpcurl', () => {
            const chain = WalletProvider.genChainFromName('bsc', customRpcUrls.bsc);
            const wp = new WalletProvider(pk, { bsc: chain });

            const client = wp.getPublicClient('bsc');
            expect(client.chain.id).toEqual(bsc.id);
            expect(client.chain.rpcUrls.default.http[0]).toEqual(bsc.rpcUrls.default.http[0]);
            expect(client.chain.rpcUrls.custom.http[0]).toEqual(customRpcUrls.bsc);
